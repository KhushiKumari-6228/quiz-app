...
export class TakeTestComponent {
  ...
  interval:any;// Initialize time remaining
  ...
  startTimer(){
    this.interval = setInterval(() => {
      if (this.timeRemaining > 0) {
        ...
        this.submitAnswers(); // Automatically submit when time is up
        this.message.error('Time is up! Your test has been submitted automatically.', { nzDuration: 5000 });
      }}
    },1000);
...
submitAnswers() {
  ...
  clearInterval(this.interval); // Stop the timer when submitting answers
